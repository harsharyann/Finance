...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 h-screen sticky top-0 border-r bg-white p-4">
      ...
      <div className="mt-auto pt-4 border-t space-y-4">
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          {user?.user_metadata?.avatar_url ? (
            <img src={user.user_metadata.avatar_url} className="w-8 h-8 rounded-full border" alt="User" />
          ) : (
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate">{user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Loading...'}</p>
